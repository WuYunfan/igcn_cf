...
def fitness(lr, l2_reg, contrastive_reg):
    set_seed(2021)
    ...
    trainer_config = {'name': 'IDCFTrainer', 'optimizer': 'Adam', 'lr': lr, 'l2_reg': l2_reg,
                      'contrastive_reg': contrastive_reg, 'device': device, 'n_epochs': 1000, 'batch_size': 2048,
...
def main():
    ...
    param_grid = {'lr': [1.e-3], 'l2_reg': [0., 1.e-5, 1.e-4], 'contrastive_reg': [1.e-4, 1.e-3, 1.e-2]}
    grid = ParameterGrid(param_grid)
    max_ndcg = -np.inf
    best_params = None
    for params in grid:
        ndcg = fitness(params['lr'], params['l2_reg'], params['contrastive_reg'])
